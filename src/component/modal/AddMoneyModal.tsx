...
const AddMoneyModal = ({isModal,onToggleModal} : AddMoneyModalProps) => {
  const [number,setNumber]=useState("")
  const [desc,setDesc]=useState("")
  const state=useContext(appContext)

  const handleSubmit=async()=>{
    createHistory({_id:Date.now().toString(),amount:number,desc,date:Date.now().toString(),type:"INCOME"})
    ...
    state?.dispatch({type:ONE_HISTORY,payload:{amount:number,desc,type:'INCOME'}})
    onToggleModal()
  }
